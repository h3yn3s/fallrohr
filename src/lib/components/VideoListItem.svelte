...
<div
	...
	class="relative flex cursor-pointer items-center gap-3 rounded-lg bg-base-200 p-1.5 transition hover:bg-base-300"
...
>
	<div class="relative h-16 w-28 flex-shrink-0 overflow-hidden rounded">
		...
		{#if isQueued}
			...
			<div class="absolute inset-0 flex items-center justify-center">
				{#if downloadJob?.status === 'queued'}
					<span class="badge badge-ghost badge-xs">Queued</span>
				{:else}
		...
		{#if watchPercent > 0 && !watched && !isQueued}
			<div class="absolute bottom-0 left-0 h-0.5 bg-primary" style="width:{watchPercent}%"></div>
		{/if}
		{#if duration > 0}
			<span
